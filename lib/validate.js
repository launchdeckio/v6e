 ...
 * @returns {Promise}
 */
const validateObject = async (fields, rules, options) => {

    const illegalErrors = options.strict ? evaluateIllegal(fields, rules) : {};

    const validationErrors = await props(mapValues(rules, (rule, field) => {
        return validate(rule, get(fields, field), fields, options);
    }));

    return assign(illegalErrors, validationErrors);
};
 ...
 * @returns {*}
 */
const validate = async (rule, value, attributes = null, options = {}) => {
    ...
    if (isFunction(rule)) {
        ...
        try {
            ...
            const result = await pTry(() => rule(value, attributes, ...args));
