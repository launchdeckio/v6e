 ...
 * @returns string[]
 */
const illegal = (fields, rules) => difference(keys(fields), keys(rules));

const validateObject = (fields, rules, options) => {

    const errors = options.strict ? fromPairs(map(illegal(fields, rules), field => [field, 'Illegal attribute.'])) : {};
